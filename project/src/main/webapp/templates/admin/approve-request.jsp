...
<div class="lang-bar">
    <div class="lang-bar-images">
        <a href="<c:url value="/approve?lang=ua"/>">
            <img src="templates/img/ua-01.png" alt="ua">
        </a>
        <a href="<c:url value="/approve?lang=en"/>">
...
<form action="<c:url value="/approve"/>" method="post" class="m-4">
    <div class="form-row">
        <div class="form-group col-1">
            <label for="id">#</label>
            <input type="number" value="<%=req.getId()%>" class="form-control" id="id" name="id" min="<%=req.getId()%>"
                   max="<%=req.getId()%>">
        </div>
        <div class="form-group col-1">
        ...
        <div class="form-group col-2">
            ...
            <input type="text" id="start-date" class="form-control" value="<%=req.getStartDate()%>" name="start-date"
                   disabled/>
        </div>
        <div class="form-group col-2">
    ...
    <button type="submit" class="btn btn-primary"><%=Localization.getString("approve-btn")%>
    </button>
</form>
...
<nav aria-label="my-nav">
    <ul class="pagination">
        ...
        <li class="page-item active"><a class="page-link"
                                        href="<c:url value="/approve?page=<%=i%>&method=approve&id=<%=req.getId()%>"/>"><%=i%>
        </a></li>
        <%} else {%>
