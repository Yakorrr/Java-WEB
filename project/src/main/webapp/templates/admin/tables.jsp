...
<div class="lang-bar">
    <div class="lang-bar-images">
        <a href="<c:url value="/admin-tables?lang=ua"/>">
            <img src="templates/img/ua-01.png" alt="ua">
        </a>
        <a href="<c:url value="/admin-tables?lang=en"/>">
            <img src="templates/img/us-01.png" alt="en">
        </a>
    </div>
</div>
<br>
<h2><%=Localization.getString("user-bills-table-title")%>
</h2>
<table class="table">
    <thead>
    <tr>
...
<nav aria-label="my-nav">
    <ul class="pagination">
        ...
        <li class="page-item active"><a class="page-link"
                                        href="<c:url value="/admin-tables?page-rooms=<%=(Integer) request.getAttribute("active-page-rooms")%>&page-bills=<%=i%>"/>"><%=i%>
        </a></li>
        <%} else {%>
...
<br>
<h2><%=Localization.getString("room-title")%>
</h2>
<table class="table">
    <thead>
    <tr>
    ...
    <ul class="pagination">
        <% pageCount = (Integer) request.getAttribute("page-count-rooms");
            ...
            for (int i = 1; i <= pageCount; i++) {
                if (i == active) {%>
        <li class="page-item active"><a class="page-link"
                                        href="<c:url value="/admin-tables?page-rooms=<%=i%>&&page-bills=<%=(Integer) request.getAttribute("active-page-bills")%>"/>"><%=i%>
        </a></li>
        <%} else {%>
        <li class="page-item"><a class="page-link"
                                 href="<c:url value="/admin-tables?page-rooms=<%=i%>&&page-bills=<%=(Integer) request.getAttribute("active-page-bills")%>"/>"><%=i%>
...
<div class="m-4">
    <h3><%=Localization.getString("add-room")%>
    </h3></div>
<form action="<c:url value="/admin-update-rooms"/>" method="post" class="m-4">
    <div class="form-row">
